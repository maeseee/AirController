...
class DailyTest {
    ...
    void shouldCalculateFreshAirConfidenceAccordingToTime(String dateTimeString, double expectedResult) {
        ...
        try (MockedStatic<ZonedDateTime> mocked = mockStatic(ZonedDateTime.class)) {
            mocked.when(() -> ZonedDateTime.now((ZoneId) any())).thenReturn(dateTime);
    ...
    @Test
    void shouldCalculateFreshAirConfidenceWithLocalTimezone() {
        final ZonedDateTime dateTime = ZonedDateTime.of(2026, 7, 24, 16, 0, 0, 0, ZoneId.of("Europe/Berlin"));
        final Daily testee = new Daily();
        try (MockedStatic<ZonedDateTime> mocked = mockStatic(ZonedDateTime.class)) {
            mocked.when(() -> ZonedDateTime.now((ZoneId) any())).thenReturn(dateTime);
            ...
            assertThat(result.value()).isCloseTo(-1.0 * Daily.CONFIDENCE_WEIGHT, within(0.01));
        }
    }
}